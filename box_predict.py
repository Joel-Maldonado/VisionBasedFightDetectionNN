...
VIOLENCE_MODEL = tf.keras.models.load_model(
    'Models/Violence_Acc_0.9865145087242126.h5'
    )


VIOLENCE_DIR = '/u/jruiz_intern/jruiz/Downloads/RWF-2000/val/Fight'
NON_VIOLENCE_DIR = '/u/jruiz_intern/jruiz/Downloads/RWF-2000/val/NonFight'
...
for category_num, category in enumerate([VIOLENCE_DIR, NON_VIOLENCE_DIR]):
    ...
    for vid_num, video in enumerate(videos[:NUM_VIDS_PER_CATEGORY]):
        ...
        for i, frame in enumerate(frames):
            ...
            cv2.putText(img, f"Violent: {(violence * 100):.2f}%", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)

            processed_frames.append(cv2.resize(img, (w, h)))

        out = cv2.VideoWriter(f'Predictions/{vid_name}_LABELED.avi', cv2.VideoWriter_fourcc(*'DIVX'), fps, (w, h))
        ...
        print(f"Finished with Video: {vid_num+1}/{NUM_VIDS_PER_CATEGORY} | Category: {category_num+1}/{2}")

# vid_path = '/u/jruiz_intern/jruiz/Downloads/UBI_FIGHTS/videos/fight/F_20_1_1_0_0.mp4'
# IMG_SIZE = 256
...
# roll_violence_preds = get_rolling_average(violence_preds, window_size=3)
