...
class RandomRotationVideo(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        random_factor = (
            tf.random.uniform(()) * self.max_rotation * 2 - self.max_rotation
        )
    ...
    return (
        tf.map_fn(
            lambda x: tf.image.resize(
                tf.image.resize(x, (int(IMG_SIZE / factor), int(IMG_SIZE / factor))),
                (IMG_SIZE, IMG_SIZE),
            ),
            video,
        ),
...
reduce_lr_on_plataeu = tf.keras.callbacks.ReduceLROnPlateau(
    monitor="val_accuracy", factor=0.2, patience=5
)
...
acc_cp = tf.keras.callbacks.ModelCheckpoint(
    f"Checkpoints/violence_model_acc_{time_date}.h5",
    save_best_only=True,
    monitor="val_accuracy",
)
loss_cp = tf.keras.callbacks.ModelCheckpoint(
    f"Checkpoints/violence_model_loss_{time_date}.h5",
    save_best_only=True,
    monitor="val_loss",
)
...
history = model.fit(
    ...
    callbacks=[
        ...
        tf.keras.callbacks.TensorBoard("tb_logs1"),
    ],
