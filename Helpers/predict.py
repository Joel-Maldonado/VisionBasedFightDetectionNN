...
while ret:
    ...
    pred_frame = cv2.resize(
        pred_frame,
        (IMG_SIZE * 3, int(IMG_SIZE * ratio) * 3),
...
for normal_frame in normal_frames:
    ...
    if violent_binary == 1:
        ...
        cv2.rectangle(
            normal_frame, (x1, y1), (x2, y2), (0, 255, 0), 2, lineType=cv2.LINE_AA
        )
    ...
    cv2.putText(
        normal_frame,
        str(int(violent_pred * 100)) + "%",
        (width, 30),
        FONT,
        FONT_SCALE,
        np.round(secondary_color * BACKGROUND_DIV).tolist(),
        FONT_THICKNESS * 2,
        lineType=cv2.LINE_AA,
    )
    cv2.putText(
    ...
    cv2.putText(
        normal_frame,
        txt,
        (10, TEXT_TOP + MARGIN * 2),
        FONT,
        FONT_SCALE,
        np.round(primary_color * BACKGROUND_DIV).tolist(),
        FONT_THICKNESS * 2,
        lineType=cv2.LINE_AA,
    )
    cv2.putText(
...
writer = cv2.VideoWriter(
    f"{video_name}_LABELED.avi", cv2.VideoWriter_fourcc(*"DIVX"), fps, (w, h)
)
